...
# Adopted from https://github.com/applied-ai-lab/genesis/blob/master/third_party/sylvester/VAE.py
...
from .gated_conv import GatedConv2d, GatedConvTranspose2d
...
def build_gc_decoder(cin, cout, stride, zdim, kz, hn=None, gn=None):
    ...
    for l, (i, o, s) in enumerate(zip(cin, cout, stride)):
        layers.append(
            GatedConvTranspose2d(i, o, 5, s, 2, s - 1, h_norm=hn, g_norm=gn))
    return nn.Sequential(*layers)
...
class VAE(nn.Module):
    ...
    def __init__(self, z_size, input_size, nout, h_size=256, enc_norm=None, dec_norm=None):
    ...
    def create_encoder(self, cin, cout, strides):
        ...
        q_z_nn = build_gc_encoder(cin,
                                  cout,
                                  strides,
                                  self.q_z_nn_output_dim,
